...
"
" Modified By:  Colin Pronovost <colin.pron@live.com> on 2017-09-13
"------------------------------------------------------------------------
" Description:
...
let s:re_other = '\(.*\)$'
let s:re_sub   = s:re_var . '\s*=\s*' . s:re_val . s:re_other
...
function! lh#let_modeline#_parse_line(mtch) abort
  ...
  let mtch  = a:mtch
  while strlen(mtch) != 0
    let vari = substitute( mtch, s:re_sub, '\1', '' )
    let valu = substitute( mtch, s:re_sub, '\2', '' )
    ...
    let mtch = substitute( mtch, s:re_sub, '\3', '' )
